package com.product_service.repository;

import java.time.LocalDateTime;
import java.util.List;

import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.data.jpa.repository.Modifying;
import org.springframework.data.jpa.repository.Query;
import org.springframework.data.repository.query.Param;

import com.product_service.entity.OutboxEvent;

public interface OutboxEventRepository extends JpaRepository<OutboxEvent, Long>{
	
	List<OutboxEvent> findByStatus(String status);
	
	@Modifying
	@Query(
	    value = "UPDATE outbox_events SET status = 'PROCESSING',processing_started_at = CURRENT_TIMESTAMP WHERE id = :id AND status = 'PENDING'",
	    nativeQuery = true
	)
	int claimEvent(@Param("id") Long id);
	
	
	@Modifying
	@Query(value="Update outbox_events set status='PUBLISHED' WHERE ID= :id",nativeQuery = true)
	int markPublished(Long id);
	
	
	List<OutboxEvent> findByStatusAndProcessingStartedAtBefore(
	        String status,
	        LocalDateTime time
	);
	
	@Modifying
	@Query(value = "DELETE FROM outbox_events where status='PUBLISHED'",nativeQuery = true)
	int deletePublishedEvents();
	

}
